      ...
      appBar: AppBar(
        ...
        actions: [
          GetBuilder<ControllerFruitStore>(
            ...
            init: controller,
            builder: (controller) {
              // ĐÃ SỬA: Bọc Badge vào IconButton để có thể bấm được
              return IconButton(
                onPressed: () {
        ...
        onPressed: () async {
          if (supabase.auth.currentUser == null) {
            ...
            if (supabase.auth.currentUser != null) {
              ...
              // Tạm thời comment dòng này lại nếu bạn không muốn chuyển thẳng sang trang thông tin sau khi đăng nhập xong
              // Get.to(() => const PageThongTinUser());
            }
          } else {
            await controller.themMH_vao_GH(fruit);
            ScaffoldMessenger.of(context).showSnackBar(
...
// ĐÃ THÊM: Giao diện giả lập giỏ hàng để bạn thiết kế tiếp
